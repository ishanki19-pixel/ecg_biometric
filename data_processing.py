...
class csvGenerator:
  def __init__(self):
    ...
    # here,os.getcwd():it is used to return the current working directory 
    self.database = 'ecgiddb'
        
  def constructor(self, folder, filename):
    signals, fields = wfdb.rdsamp(filename, sampfrom=0, pn_dir=os.path.join(self.database, folder))
          ...
          if (files.endswith('csv')):
           with open(os.path.join(filepath, folders, files), "r") as x:
              ...
              for rows in range(len(pdfeats)): 
                temp.append(pdfeats._get_value(rows, 1, True))
                temp1.append(pdfeats._get_value(rows, 0, True))
...
class Augmentation:
  ...
  def init(self):
    ...
    for row in range(len(dfmaxs)):
      dists = [self.dist(p1,p2) for p1, p2 in combinations(dfmaxs._get_value(row,0,True), 2)]
